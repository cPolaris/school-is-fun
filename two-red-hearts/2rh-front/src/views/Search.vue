<template>
  <div>
    <md-layout md-align="center">
      <md-card md-with-hover id="search-card">
        <md-card-header>
          <div class="md-title">Search</div>
          <div class="md-subhead">Directly search for users matching criteria. Gender and major are exact match. Name and traits are close-match. OR logic is applied if multiple fields are specified</div>
        </md-card-header>
        ...
        <md-card-content>
          <form @submit.prevent novalidate>
            ...
            <label>Gender</label>
            <div>
              <md-radio v-model="genderCode" name="gender-radio" md-value="f">Female</md-radio>
              <md-radio v-model="genderCode" name="gender-radio" md-value="m">Male</md-radio>
              <md-radio v-model="genderCode" name="gender-radio" md-value="o">Other</md-radio>
            </div>

            <md-input-container><label>Major</label>
              <md-select name="major-selection" v-model="majorCode">
            ...
            <md-checkbox v-model="tagboxes[6]">Seriouly social 🎉</md-checkbox>
            <md-checkbox v-model="tagboxes[7]">एÖl丫glДヒ (polyglot)</md-checkbox>
            <md-checkbox v-model="tagboxes[8]">Gym = ❤️</md-checkbox>
            <md-checkbox v-model="tagboxes[9]">Runner</md-checkbox>
            <md-checkbox v-model="tagboxes[10]">🍔🥓🥓🥓🌮🌯🍖🍖🍖🍔</md-checkbox>
            <md-checkbox v-model="tagboxes[11]">🍎🍊🍋🍌🍉🍇🍓🥑🥗</md-checkbox>
            <md-checkbox v-model="tagboxes[12]">🎺🎸🎻🥁🎷🎹🎼</md-checkbox>
            <md-checkbox v-model="tagboxes[13]">🏀⚽️🏈⚾️🎾🏐🏓🏸🏒</md-checkbox>
            <md-checkbox v-model="tagboxes[14]">🐶🐱🐕🐈🐢🕊🐇🦎🦄 (pets)</md-checkbox>
        ...
        <md-card-actions>
          <md-button class="md-raised md-primary" name="result-card" @click.native="doSearch">Search</md-button>
        </md-card-actions>
        <md-progress md-indeterminate v-if="working"></md-progress>
      </md-card>

      <md-table-card id="result-card">
        <md-table>
          <md-table-header>
            <md-table-row>
              ...
              <md-table-head>Trait Similarity</md-table-head>
          ...
          <md-table-body>
            <md-table-row v-for="row in result" :key="row.id">
              ...
              <md-table-cell>{{ row.similarity }}</md-table-cell>
...
<script>
  export default {
    ...
    methods: {
      doSearch () {
        this.$http.post('/api/usersearch', {
          name: this.name,
          genderCode: this.genderCode,
          majorCode: this.majorCode,
          hobbyTags: this.tagboxes
        }).then(res => {
          const resultRows = JSON.parse(res.bodyText);
          this.result.length = 0;
          for (let rw of resultRows) {
            this.result.push(rw);
          }
        }, err => {
